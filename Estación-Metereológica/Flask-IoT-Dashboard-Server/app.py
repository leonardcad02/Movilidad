...
logged_in = {}
api_loggers = {}
mydb = database.db('root', 'localhost', '', 'movilidad_db')
...
#get all the user information from the device
...
#get all the api key information from the users
...
#get all the apikey information from the apiuser
...
def devicestat (apikey, fieldname, deviceID):
    ...
    if not(apikey in api_loggers):
        ...
        return jsonify (data)
#get all the data information from the station 
    ...
    try:
        ...
        if output:
            if (len(data) == 10) and (type(data) is list):
                ...
                pmtwo = data[8]
                pmten = data[9]
                mydb.update_values(output[0], fieldname, deviceID, temperature, humidity, polution, noise, uv, pmone, pmtwo, pmten)
...
# Get method from information of four data of station
...
@app.route("/api/<string:apikey>/polution", methods=["GET", "POST"])
...
@app.route("/api/<string:apikey>/noise", methods=["GET", "POST"])
